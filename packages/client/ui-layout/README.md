...
The Web entry can mark an allowlisted root iframe with `data-dsh-orchestrator-chooser="true"` before mounting. This chooser keeps the native sidebar expanded by default at narrow widths, capped to the actual iframe width; below the preferred sidebar width the conversation yields to zero width. Its native toggle still works, and widening restores the preferred sidebar width without rewriting it. Normal roots retain auto-collapse below 1024px, and `data-dsh-surface="embed"` always hides the sidebar. See the [selection note](../../../.agents/notes/implemented/bug-fix/2026-09-18-orchestrator-selection.md) for opt-in and receipt semantics.

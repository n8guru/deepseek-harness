...
在明确启用的编排会话选择器中，Workspace 行操作无需悬停即可显示。即使未选择 Session 且没有最近使用的 Workspace，原生 **New session in {workspace}** 操作也会提供显式 Workspace。全局 New Session 操作保留正常回退行为；若没有任何已注册 Workspace，请使用原生添加流程或完整 DSH 选择目录。
